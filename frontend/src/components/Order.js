...
class Order extends React.Component {
  ...
  render() {
    let formattedDate =
      this.props.order.date.split("T")[0] +
      " " +
      this.props.order.date.split("T")[1].split(".")[0];

    const modalTrigger = (
      <List.Item onClick={this.handleModalOpen}>
        <List.Icon name="tag" size="large" verticalAlign="middle" />
        <List.Content>
          <List.Header as="a">Order #{this.props.order.id}</List.Header>
      ...
      >
        ...
        <Modal.Content>
          <Input
            ...
            labelPosition="right"
            value={
              this.props.order.delivery_method.charAt(0).toUpperCase() +
              this.props.order.delivery_method.slice(1)
            }
            fluid={true}
          />
          ...
          <Input
            ...
            labelPosition="right"
            value={
              this.props.order.payment_method.charAt(0).toUpperCase() +
              this.props.order.payment_method.slice(1)
            }
            fluid={true}
          />
          ...
          <Input
            label={{ tag: true, content: "Expected Delivery" }}
            labelPosition="right"
            value={formattedDate}
            fluid={true}
          />
          ...
          <Button
            ...
            label={{
              basic: true,
              pointing: "left",
              content: this.props.order.total_price
            }}
          />
        </Modal.Content>
      </Modal>
