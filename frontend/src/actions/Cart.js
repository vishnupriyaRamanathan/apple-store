...
export const placeOrder = (items, shippingData, billingData) => {
    return dispatch => {
        ...
        _.forEach(items, (element) => {
            ...
            axios.put(BASE_URL+`/api/products/${element.product.id}/`, {category: element.product.category, name: element.product.name, price: element.product.price, quantity: updatedQuantity})
        })

        axios.post(BASE_URL+'/api/orders/', {products: productsArr, quantities: quantitiesArr, total_price: totalPrice.toFixed(2), delivery_method: shippingData.deliveryMethod, payment_method: billingData.paymentMethod})
            .then((res) => {
